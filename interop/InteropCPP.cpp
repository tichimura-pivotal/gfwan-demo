...
int main(int argc, char ** argv)
{
  try
  {
    ...
    {
      LOGINFO("CPP CLIENT: Checking server for this CPP CLIENT keys...");
      value1 = regionPtr->get("Key-CPP");
      LOGINFO("CPP CLIENT: Checking server for JAVA CLIENT keys...");
      value2 = regionPtr->get("Key-JAVA");
      gemfire::millisleep(1000);
    }

    LOGINFO("CPP CLIENT: Key-CPP value is %s", value1->toString()->asChar());
    LOGINFO("CPP CLIENT: Key-JAVA value is %s", value2->toString()->asChar());
